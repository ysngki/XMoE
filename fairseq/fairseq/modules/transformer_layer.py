...
from .mlp_gating import Experts, TopKGate
...
class MLPLayer(nn.Module):
    ...
    def forward(self, hiddden_states):
        x = self.activation_fn(self.fc1(hiddden_states))
        x = self.activation_dropout_module(x)
        x = self.fc2(x)
...
class TransformerEncoderMoELayerBase(nn.Module):
    """Encoder layer block.
    ...
    *cfg.encoder.normalize_before* to ``True``.
    ...
    def _get_fc_rank(self, remove_num: int) -> List[int]:
        raise Exception("YYH here Prohibit!")
    ...
    def experts_forward(self, x):
        ...
        x = expert_output.reshape(x.shape)

        return x, l_aux, gate_info
    ...
    def forward(
        ...
        # todo
        x, l_aux, gate_info = self.experts_forward(x)
        ...
        if self.return_fc and not torch.jit.is_scripting():
            return x, fc_result, l_aux, gate_info
        return x, l_aux, gate_info
...
class DecoderMLPLayer(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        self.embed_dim = cfg.decoder.embed_dim
        self.quant_noise = cfg.quant_noise.pq
...
class TransformerDecoderMoELayerBase(nn.Module):
    """Decoder layer block.
    ...
    def forward(
        ...
        if self.onnx_trace and incremental_state is not None:
            ...
            if self_attn_padding_mask is not None:
                ...
                self_attn_state = [saved_state["prev_key"], saved_state["prev_value"]]
            return x, attn, self_attn_state, l_aux, gate_info
        return x, attn, None, l_aux, gate_info
